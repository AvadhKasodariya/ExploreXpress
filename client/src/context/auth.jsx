...
export default function ({ children }) {
    ...
    const loadSession = () => {
        let authSession = localStorage.getItem('auth-session-user')
    ...
    const clearSession = () => {
        ...
        localStorage.removeItem('auth-session-user')
    }

    const setSession = (session) => {
        setAuthSession(session)
        axios.defaults.headers.common = { 'Authorization': `bearer ${session.token}` }
        localStorage.setItem('auth-session-user', JSON.stringify(session))
    }
